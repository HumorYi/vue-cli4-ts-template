import { isIEBrowser } from './verify'
import Message from './message'
const message = new Message()

export default class Download {
  error(filenamePrefix: string = ''): Error {
  ...
  getFilenameSuffix(contentDisposition: string): string {
    return contentDisposition.length >= 2 ? contentDisposition.split('=')[1] : ''
  }
  ...
  download(blob: Blob, filename: string): void {
    isIEBrowser() ? this.forIEBrowser(blob, filename) : this.forMainstreamBrowser(blob, filename)
  }
}
