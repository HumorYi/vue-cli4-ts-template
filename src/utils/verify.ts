/* eslint-disable */
const isIEBrowser = (): boolean => typeof window.navigator.msSaveOrOpenBlob === 'function'

const isIncludeCrossbarChar = (val: any): boolean => String(val).includes('-')

const isInteger = (val: any): boolean => isPositiveNumber(val) && Number.isSafeInteger(val)

const isPositiveNumber = (val: any): boolean =>
  !isIncludeCrossbarChar(val) && typeof val === 'number' && !isNaN(val) && val >= 0

const isPercent = (val: any): boolean => isPositiveNumber(val) && val >= 0 && val <= 100

const isNullObject = (val: object): boolean => JSON.stringify(val) === '{}'

const isMobile = (val: any): boolean => /^1[3456789]\d{9}$/.test(val)

const isEmail = (val: any): boolean => /^[\w.-]+@([a-zA-Z\d-]+\.)+[a-zA-Z\d]{2,4}$/.test(val)
...
const isAndroid = (): boolean =>
  window.navigator.userAgent.includes('Android') || window.navigator.userAgent.includes('Linux')

const isUrl = (val: any): boolean =>
  /^((ht|f)tps?):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:\/~\+#]*[\w\-\@?^=%&\/~\+#])?$/i.test(val)

const isPhone = (val: any): boolean =>
  /^((\d{8})|(\d{3, 4})-(\d{7,8})|(\d{3, 4})-(\d{7,8})-(\d{1, 4})|(\d{7,8})-(\d{1, 4}))$/.test(val)

const isPositiveInt = (val: any): boolean => /^[1-9]\d*$/.test(val)

const isNonnegativeInt = (val: any): boolean => /^[1-9]\d*|0$/.test(val)

const isFloat = (val: any): boolean => /^[1-9]\d*\.\d*|0\.\d*[1-9]\d*$/.test(val)
...
const isRoom = (val: any): boolean => /^[\u4e00-\u9fa5_a-zA-Z1-9_][\u4e00-\u9fa5_a-zA-Z0-9_]{0,10}$/.test(val)
